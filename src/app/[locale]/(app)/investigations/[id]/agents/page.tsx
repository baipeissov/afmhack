...
// understands WHY there are several of them. Names/roles come from the
// warRoom.agentMeta translation namespace, keyed by agent id.
  ...
  useEffect(() => {
    if (state.signals.length > seenSignals.current) {
      ...
      toast.error(t("toastTitle"), {
        description: t("toastDescription", { topic: sig.topic }),
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-start gap-3 border-b border-border-subtle bg-bg-surface-1 px-5 py-3">
        ...
        <p className="text-sm text-text-secondary">
          ...
          {t("explainer1")}{" "}
          <span className="text-contradiction">{t("explainer2")}</span>
        </p>
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          >
            <RotateCcw className="size-3.5" /> {t("replayDemo")}
          </button>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="grid min-w-0 flex-1 grid-cols-1 divide-x divide-border-subtle md:grid-cols-3">
          {agentIds.map((aid) => {
            const color = AGENT_COLOR[aid];
            const name = t(`agentMeta.${aid}.name`);
            const role = t(`agentMeta.${aid}.role`);
                ...
                <div className="border-b border-border-subtle bg-bg-surface-1 px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-[11px] text-text-muted">{role}</div>
                    </div>
                  </div>
                </div>
                  ...
                  {msgs.map((m) => (
                    ...
                    >
                      <div className="mb-0.5 text-[10px] font-medium uppercase tracking-wide text-text-muted">
                        {m.speaker === "agent" ? t("agentLabel") : t("suspectLabel")} · {m.ts}
                      </div>
                      {m.text}
                      {m.flag && (
                        <div className="mt-1.5 flex items-center gap-1.5 rounded bg-manipulation/15 px-1.5 py-1 text-[11px] font-medium text-manipulation">
                          ...
                          {t("manipulationLabel")}: {techniqueLabels[m.flag]}
                        </div>
                      )}
                    </div>
                  ))}
        ...
        <aside className="hidden w-72 shrink-0 flex-col overflow-y-auto border-l border-border-subtle bg-bg-surface-1 p-4 lg:flex">
          <div className="mb-1 text-sm font-semibold text-text-primary">{t("liesCaught")}</div>
              ...
              {state.signals.map((sig, i) => (
                ...
                >
                  <div className="flex items-center gap-1.5 text-sm font-semibold text-contradiction">
                    <ShieldAlert className="size-4" /> {t("lieNumber", { n: i + 1 })}: {sig.topic}
                  </div>
                  <div className="mt-1.5 font-mono text-xs text-text-secondary">
                    {sig.quote}
                  </div>
                  <div className="mt-1.5 text-[11px] text-text-muted">
                    {t("caughtBecause", { agent: sig.with })}
                  </div>
                </div>
              ))}
            </div>
